...
MThreadPool::MThreadPool(size_t threads)
    : active_threads_(0),
      stop_(false)
{
  for (size_t i = 0; i < threads; ++i)
    workers_.emplace_back(std::bind(&MThreadPool::run, this));
}
...
void MThreadPool::run()
{
  while (true) {
    ...
    while (!this->stop_ && this->tasks_.empty()) {  // enqueue() add std::function<void()> into tasks_
      this->tasks_condition_.wait(lock);            // Atomically unlocks lock, block until notify in enqueue()
    }
...
void MThreadPool::waitForEmptyQueue() const
{
  std::unique_lock<std::mutex> lock(this->tasks_mutex_);
  // Only exit if all tasks are complete by tracking the number of
  // active threads.
  while (active_threads_ || !tasks_.empty()) {
    this->wait_condition_.wait(lock);   // notify_all() in run() after one task() is executed
  }
}
