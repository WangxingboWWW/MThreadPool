...
class MThreadPool
{
 ...
 private:
  ...
  /// The task queue.
  std::queue<std::function<void()>> tasks_;  // void f(void)
...
template<class Function, class ... Args>
std::future<typename std::result_of<Function(Args...)>::type> MThreadPool::enqueue(
    Function&& function, Args&&... args)
{
  ...
  {
    std::unique_lock<std::mutex> lock(tasks_mutex_);  // unique_lock
    tasks_.push([task]() {(*task)();});  // lambda, capture task and push *task into queue
  }
  tasks_condition_.notify_one();  // notify
  return res;
}
